...
// Creeaza un header de tip iphdr cu valorile primite ca parametru
void create_ip_hdr(struct iphdr *ip_hdr, uint32_t daddr, uint32_t saddr){
...
// Pune intr-un pachet dat ce va fi trimis ca o eroare ICMP valorile primite ca parametru
void create_icmp_error(packet* packet, uint32_t daddr, uint32_t saddr, uint8_t *shost, uint8_t *dhost, u_int8_t type){
	
	//Costruiesc struct ether_header
	struct ether_header aux_eth_hdr;
	memcpy(aux_eth_hdr.ether_dhost, shost, ETH_ALEN);
	memcpy(aux_eth_hdr.ether_shost, dhost, ETH_ALEN);
	aux_eth_hdr.ether_type = htons(ETHERTYPE_IP);

	//Costruiesc struct iphdr
	struct iphdr aux_ip_hdr;
	create_ip_hdr(&aux_ip_hdr, daddr, saddr);

	//Costruiesc struct icmphdr
	struct icmphdr aux_icmp_hdr;
	aux_icmp_hdr.type = type;
	aux_icmp_hdr.code = 0;
